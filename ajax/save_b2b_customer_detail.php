<?php
    ...
    $admin= new Admin();
		...
		if($customer_phone != "")
		{
			...
			{
				$error_msg = "Phone number can not be 10 digit in length."."<br>";
				echo json_encode(array("status" => 0,"msg" => $error_msg ,  "response" => "fail" ));
				exit;
			}
		}
					...
					if(isset($b2b_cart_data) and !empty($b2b_cart_data)) {
						...
						foreach($cartarray as $product=>$val) {
                            ...
                            <tr>
								...
								<span style='font-size:13px;font-weight:600;line-height:1.4;color:#555'>".$val['p_code']." - ".$val['product_name']."</span>".$preorder_text." 
								</td>
								<td style='font-family: arial; width:100%;'>
								...
								if(!empty($customer_comment))	{
									...
									$orderContent .= "<div>".$customer_comment."</div>";
								}
								...
								<td width='43%'>&nbsp;</td>
									...
									$str .= ' <div class="col-sm-6 custo-detail">
												...
												<table><tbody>
													<tr><td>Name</td><td>'.ucfirst($customer_name).'</td></tr>';
													
													if($customer_email != ""){
														$str .= '<tr><td>Email</td><td>'.$customer_email.'</td></tr>';
													}

													if($customer_phone != ""){
														$str .= '<tr><td>Contact No.</td><td>'.$customer_phone.'</td></tr>';
													}
													
													if($customer_business_name != ""){
														$str .= '<tr><td>Company</td><td>'.$customer_business_name.'</td></tr>';
													}
													
													if($customer_address != ""){
														$str .= '<tr><td>Address</td><td>'.$customer_address.'</td></tr>';
													}

													if($customer_comment != ""){
														$str .= '<tr><td>Comments</td><td>'.$customer_comment.'</td></tr>';
													}
											...
											foreach($b2b_cart_data as $key=>$myval){
												...
												$str .= '<tr>
													<td>'.$k.'</td>
													<td class="txt-al">'.$val->p_code.' - '.$val->product_name.$preorder_text.'</td>
													<td>'.$val->item_qty.'</td>
													<td>'.$tot_amount.'</td>
												</tr>';
												$k++;
